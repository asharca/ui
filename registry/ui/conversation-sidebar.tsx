...
export function ConversationSidebar({ groups, activeConversationId, onSelect, onNew, onRename, onDelete, onConversationOrderChange, className }: ConversationSidebarProps) {
  ...
  return <section aria-label="会话列表" className={cn('flex h-full min-h-0 flex-col bg-background text-sm', className)}>
    ...
    <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-2.5">{filtered.map((group) => group.visible.length > 0 && <section key={group.id}><h3 className="px-2 pb-2 text-[10px] text-muted-foreground">{group.label}</h3><div className="grid gap-1">
      {group.visible.map((item, index) => <div key={item.id} className={cn('group flex items-center rounded-lg hover:bg-muted/50', activeConversationId === item.id && 'bg-muted')}
    ...
    </div></section>)}{filtered.every((group) => !group.visible.length) && <p className="px-4 py-10 text-center text-xs text-muted-foreground">没有匹配的会话。</p>}</div>
