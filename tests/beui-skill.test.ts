...
function slugsFromSkill(markdown: string) {
  ...
  for (const match of markdown.matchAll(/@beui\/([a-z][a-z0-9-]*)/g)) found.add(match[1]);
  for (const match of markdown.matchAll(/https:\/\/beui\.dev\/r\/([a-z][a-z0-9-]*)(?:\.json)?/g)) {
    if (match[1] !== "registry") found.add(match[1]);
  }
  for (const line of markdown.split("\n")) {
    if (!line.includes("|")) continue;
    for (const match of line.matchAll(/`([a-z][a-z0-9-]*)`/g)) found.add(match[1]);
  }
  return [...found];
}
...
describe("beUI skill", () => {
  ...
  test("every picker slug is a real install name", async () => {
    const skill = await readFile(SKILL_PATH, "utf8");
    const known = installSlugs();
    expect(slugsFromSkill(skill).filter((slug) => !known.has(slug))).toEqual([]);
  });
  ...
  test("local registry.json is a scrapeable catalog of install slugs", async () => {
    ...
    const skill = await readFile(SKILL_PATH, "utf8");
    expect(slugsFromSkill(skill).filter((slug) => !names.includes(slug))).toEqual([]);
  });
  ...
  test("picker evals point at real slugs", async () => {
    ...
    for (const item of evals.evals) {
      ...
      for (const slug of [...item.expected_slugs, ...item.not_slugs]) expect(known.has(slug)).toBe(true);
      expect(item.expected_slugs.filter((slug) => item.not_slugs.includes(slug))).toEqual([]);
    }
  });

  test("production registry.json can be fetched with the skill command", async () => {
    const proc = Bun.spawn(["curl", "-fsS", LIVE_REGISTRY], { stdout: "pipe", stderr: "pipe" });
    const stdout = await new Response(proc.stdout).text();
    const stderr = await new Response(proc.stderr).text();
    const code = await proc.exited;
    expect(code, stderr || `curl exited ${code}`).toBe(0);
    const catalog = JSON.parse(stdout) as { name?: string; items?: Array<{ name?: string; title?: string; description?: string }> };
