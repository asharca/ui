...
it("has real source files and public imports for every component page", () => {
  expect(new Set(componentDocs.map((doc) => doc.id)).size).toBe(
  ...
  for (const doc of componentDocs) {
    ...
    expect(
      doc.module ? `./${doc.module}` in pkg.exports : "." in pkg.exports,
      doc.name,
    ).toBe(true);
    expect(doc.preview !== null || doc.id === "chat-thread").toBe(true);
    if (doc.demoFile) {
      const demo = readFileSync(resolve("showcase/demos", doc.demoFile), "utf8");
      expect(demo).not.toContain("componentDocs");
      expect(demo).not.toContain("./ChatExample");
      expect(demo.match(/export function /g)).toHaveLength(1);
    } else {
      expect(doc.code.trim()).not.toBe("");
    }
  }
});

it("shows only the current component's demo source", async () => {
  window.history.replaceState(null, "", "/#/components/chat-shell");
  const user = userEvent.setup();
  render(<DocsApp />);
  expect(screen.queryByRole("tab", { name: "演示源码" })).not.toBeInTheDocument();
  await user.click(screen.getByRole("button", { name: "发布计划", exact: true }));
  ...
  expect(code).not.toHaveTextContent("ConversationDemo");
  expect(code).not.toHaveTextContent("WorkspaceDemo");
...
it("starts on installation and filters the component directory", async () => {
  ...
  render(<DocsApp />);
  expect(
    screen.getByRole("heading", { name: "安装", exact: true }),
  ).toBeVisible();
  expect(screen.getByRole('region', { name: '安装命令' }).querySelector('code')).toHaveTextContent(/^pnpm add @asharca\/ui$/);
  const troubleshooting = screen.getByText('依赖未自动安装或版本冲突？').closest('details');
  ...
  await user.selectOptions(
    screen.getByRole("combobox", { name: "包管理器" }),
    "npm",
  );
  expect(screen.getByRole('region', { name: '安装命令' })).toHaveTextContent('npm install @asharca/ui');
  expect(screen.getByRole('region', { name: '安装命令' }).querySelector('code')).toHaveTextContent(/^npm install @asharca\/ui$/);
  await user.type(
    screen.getByRole("searchbox", { name: "搜索组件文档" }),
    "ChatComposerToolbar",
  );
  expect(
    screen.getByRole("link", { name: "ChatComposerToolbar" }),
  ).toHaveAttribute("href", "#/components/chat-composer-toolbar");
  expect(
    screen.queryByRole("link", { name: "Accordion", exact: true }),
