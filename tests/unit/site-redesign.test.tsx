...
it.each(['/', '/#/', '/#/home'])('opens the product homepage at %s with real form controls', async (path) => {
  ...
  expect(screen.getByRole('radio', { name: 'AI 工作流', exact: true })).not.toBeChecked();
  await user.click(screen.getByRole('button', { name: '保存配置', exact: true }));
  expect(screen.getByText('配置已保存 · 仅本地演示')).toBeVisible();
  await user.click(screen.getByRole('switch', { name: '流式响应' }));
  expect(screen.queryByText('配置已保存 · 仅本地演示')).not.toBeInTheDocument();
