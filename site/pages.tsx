...
export function ComponentPage() {
  ...
  return <article>
    ...
    {entry.slug.startsWith('agent-') && <section className="doc-section"><h2>beUI Agent 源码移植</h2><p className="reading-note">此组件从 beUI 的公开 MIT 源码适配，使用独立 agent-* 安装入口，与原有 Message、PromptInput、ApprovalCard 和 ToolResult 并存。安装会带入必要的内部依赖；更新前检查完整文件清单。模型调用、持久化与真实审批由业务项目负责，示例只操作本地状态。</p><p className="reading-note">普通 AI 正文、表格和 Mermaid 继续使用 SafeStreamdown；Agent Streaming Response 可以组合它，而不是替换渲染器。</p></section>}
...
export function InstallationPage() {
  ...
  return <article>
    ...
    <section id="updating" className="doc-section"><h2>手动安装与更新</h2><p className="reading-note">每个组件页面的 Manual 标签列出全部依赖和源码文件。无论哪种安装方式，拿到的都是相同源码。更新前先检查差异，不要直接覆盖自己修改过的文件。</p><p className="reading-note">升级 npm 依赖不会带入本地组件的样式与动画改动。先保存现场，使用 --dry-run、--diff 和 --view 检查整个安装条目，再按有无定制选择覆盖或合并，验证后保留独立提交。</p><Link className="text-link" to="/docs/updating">阅读完整更新指南 <ArrowRight size={15} /></Link><p className="small-note">指南包括命名空间、公共依赖、Agent 合并提示词、验证、回退和故障排查。</p><Link className="text-link" to="/components">浏览组件 <ArrowRight size={15} /></Link></section>
  </article>;
}
