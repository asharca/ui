...
export function DocsApp() {
  ...
  </div><DialogPortal><DialogOverlay className="docs-mobile-overlay" /><DialogContent className="docs-mobile-dialog" style={{ transform: 'none', translate: '0 0' }} aria-describedby={undefined} onOpenAutoFocus={(event) => { event.preventDefault(); mobileSearch.current?.focus(); }}><div className="docs-mobile-heading"><DialogTitle>文档导航</DialogTitle><DialogClose asChild><IconButton label="关闭文档导航" variant="ghost" icon={<X size={18} />} /></DialogClose></div><nav className="docs-mobile-links" aria-label="站点入口"><a href="#/home" onClick={() => setOpen(false)}>首页</a><a href="#/examples" onClick={() => setOpen(false)}>示例</a></nav>{directory(true)}</DialogContent></DialogPortal></Dialog><SiteSearch open={searchOpen} onOpenChange={setSearchOpen} /></>;
}
