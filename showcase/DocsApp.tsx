  ...
  return <>
    ...
    <section id="preview"><Tabs value={view} onValueChange={setView} className="docs-component-tabs">
      ...
      <TabsContent value="preview" forceMount aria-hidden={view !== 'preview'} inert={view !== 'preview'} style={view === 'preview' ? undefined : { visibility: 'hidden', position: 'absolute', insetInline: 0, top: 0 }}><div className="docs-preview docs-preview-catalog" data-canvas={canvas}>{viewport === 'inline' ? <div key={reset} className="docs-preview-inner docs-demo-canvas">{doc.preview}</div>
        : <div className="docs-viewport-scroll"><iframe key={`${reset}-${viewport}`} title={`${doc.name} ${viewport}px 预览`} src={standalone} style={{ width: Number(viewport) }} className="docs-preview-frame" /></div>}</div>{view === 'preview' && <div className="studio-canvas-bar"><span>真实组件 · 可交互预览</span><div className="studio-canvas-options" role="group" aria-label="预览背景">{[['plain', '纯色'], ['dots', '点阵'], ['grid', '网格']].map(([value, label]) => <button key={value} type="button" aria-pressed={canvas === value} onClick={() => setCanvas(value)}>{label}</button>)}</div></div>}</TabsContent>
...
function Installation() {
  ...
  return <>
    <div className="docs-page-heading"><span className="docs-eyebrow">开始使用</span><h1>安装</h1><p>从安装到第一个组件。把精心设计的界面，带进你的 React 项目。</p><span className="docs-meta">文档 v{version} · {componentDocs.length} 个组件入口</span></div>
    <div className="docs-version-note">安装 npm 包前请核对实际版本与导出。旧版本可能不包含新增接口与主题；已安装项目不需要为了浏览文档升级依赖。</div>
